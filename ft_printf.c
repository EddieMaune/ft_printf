...
/*   Updated: 2018/09/24 11:11:24 by emaune           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_conv(t_var *var, const char *s)
{
	...
	while (s[i])
	{
		...
		{
			i++;
			if (is_conversion_char(s[i]) || s[i] == 'S' || s[i] == 'D')
...
void			store_args(t_var *var)
{
	...
	while (var->temp)
	{
		...
		if (var->temp->c == 'D')
		{
			var->temp->n = va_arg(var->args, long long);
		}
		if (var->temp->c == 's')
		{
		...
		if (var->temp->c == 'p')
		{
			...
			var->temp->str = ft_itoa_base(var->n, 16);
			var->t1 = var->temp->str;
			var->temp->str = ft_strjoin("0x", var->t1);
			ft_strdel(&var->t1);
		}
		if (var->temp->c == 'i')
		{
		...
		if (var->temp->c == 'S')
		{
			var->ws = va_arg(var->args, wchar_t*);
			var->temp->ws = var->ws;
		}
		if (var->temp->c == 'c')
		{
			var->n = va_arg(var->args, int);
			var->temp->str = ft_char_to_str(var->n);
		}
		if (var->temp->c == 'X')
		{
			var->n = va_arg(var->args, int);
			var->temp->str = ft_itoa_base(var->n, 16);
			ft_strtoupper(var->temp->str);
		}
		if (var->temp->c == 'o')
		{
			var->n = va_arg(var->args, int);
			var->temp->str = ft_itoa_base(var->n , 8);
		}
		var->temp = var->temp->next;
	}
}
...
void			ft_printf(const char *control_string, ...)
{
	...
	store_args(&var);
	print_output(control_string, &var);
	va_end(var.args);
}
